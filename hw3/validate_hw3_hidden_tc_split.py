...
ANS_DIR = TC_DIR
ANSWERS = [
    f"{ANS_DIR}/ans5_%d.csv",
    f"{ANS_DIR}/ans6_%d.csv",
    f"{ANS_DIR}/ans7_%d.csv",
    f"{ANS_DIR}/ans8_%d.csv",
    f"{ANS_DIR}/ans9_%d.csv"
]
...
def run_testcase(tc_number, testcase, answer):
    for i in range(1, 4):
       run_testcase_n(f'{tc_number}_{i}', testcase % i, answer % i)
# 執行測試案例
def run_testcase_n(tc_number, testcase, answer):
    try:
        # 先清空之前產生過的檔案
